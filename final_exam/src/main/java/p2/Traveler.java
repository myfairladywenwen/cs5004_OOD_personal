...
public class Traveler implements Comparable<Traveler> {
  ...
  /**
   * compares two Traveler objects based upon the total distance from Seattle,
   * that they accumulated during the last year.
   * @param otherTraveler - the other traveler to compare with
   * @return an int
   */
  @Override
  public int compareTo(Traveler otherTraveler) {
    ...
    return this.traveledDestinations.stream()
        .map(Destination::getDistanceFromSeattle)
        .reduce(0f, (x, y)-> x + y).intValue()
        - otherTraveler.traveledDestinations.stream()
        .map(Destination::getDistanceFromSeattle)
        .reduce(0f, (x, y)-> x + y).intValue();
  }
}